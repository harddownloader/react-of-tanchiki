  ...
  genereteColumns(y) {
    ...
    for (let x = 0; x < this.contextThis.state.wightMap; x++) {
      ...
      if (isOurTank.x === x && isOurTank.y === y) {
        currentBlock.push(
          <OurTank direction={this.contextThis.props.direction.directionCurrent} />,
        )
      }
      // bullets
        ...
        if (enemies[w].x === x && enemies[w].y === y) {
          currentBlock.push(
            <SelfPropelledGun
              ...
              updateEnemyXY={this.contextThis.GeneratorEnemy.updateEnemyXY.bind(this)}
              addEnemyBullet={this.contextThis.EnemyBulletsLogic.addEnemyBullet.bind(this)}
            />,
          )
        }
      }
